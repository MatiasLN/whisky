...
const productScraper = async (url) => {
	...
	let productDetails = await page.evaluate(() => {
		...
		document.body.querySelector(".product__contents-list__content-percentage")
			? (percentage = document.body.querySelector(".product__contents-list__content-percentage").textContent)
			: (percentage = "");
		...
		document.body.querySelector(".product__region").children[0]
			? (country = document.body.querySelector(".product__region").children[0].textContent)
			: (country = "");

		document.body.querySelector(".product__region").children[1]
			? (region = document.body.querySelector(".product__region").children[1].textContent)
			: (region = "Øvrige");

		let productDetailsElement = document.body.querySelectorAll(".product__tab-list > li");
