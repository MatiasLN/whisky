	...
	useEffect(() => {
		if (selected) {
			fetch("/api/singleProduct", {
				...
				.then((data) => {
					// format price
					data.price = data.price.toString().replace("Kr", "").replace(/\s+/g, "").replace(",", ".");
					...
					let changeStyle = document.querySelectorAll(".whiskyDetailsContainer");
					const searchResults = document.querySelectorAll(".searchResults");
				...
				<div id="loadingAnchor" className="searchResults">
					{isLoading ? (
						...
						<div className="displayData">
							...
							{error && (
								<>
									<p>Fant dessverre ingen produkter på Vinmonopolet som matcher søket ditt.</p>
									<button
										className="addNewBtn"
										onClick={() => {
											update({ manual: true });
